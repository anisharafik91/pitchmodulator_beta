...
//  PitchModulator
//
//  Created by Anisha Rafik on 22/03/16.
...
struct Alerts
{
    ...
    static let RecordingFailedTitle = "Failed Recording"
    ...
    func playSound (rate rate : Float? = nil,pitch : Float? = nil, echo : Bool = false, reverb: Bool = false)
    {
        ...
        //modes for audio playing
        audioPlayerNode=AVAudioPlayerNode()
        audioEngine.attachNode(audioPlayerNode)
        
        //rate_pitch adjustment node
        let changeRatePitchNode = AVAudioUnitTimePitch()
        
        //check for not nil val
        if let pitch = pitch{
        ...
        audioPlayerNode.scheduleFile(audioFile, atTime: nil)
            {
                ...
                if let lastRenderTime = self.audioPlayerNode.lastRenderTime, let playerTime = self.audioPlayerNode.playerTimeForNodeTime(lastRenderTime)
                {
                    ...
                    //schedule stop timer when audio finished playing
